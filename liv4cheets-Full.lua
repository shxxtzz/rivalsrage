...
--!strict
-- Utils.luau | shared helpers (cloneref-safe, high UNC ok, no UNC required to load)

local Utils = {}

-- cloneref if available (Potassium/Wave), else passthrough
local function cref<T>(inst: T): T
	local f = (getgenv() :: any).cloneref or (rawget(_G, "cloneref") :: any)
...
function Utils.LocalPlayer()
	return Utils.Services.Players.LocalPlayer
end

function Utils.Character(plr)
	plr = plr or Utils.LocalPlayer()
	return plr and plr.Character or nil
end

function Utils.Humanoid(char): Humanoid?
	if not char then
		return nil
	end
...
function Utils.RootPart(char): BasePart?
	if not char then
		return nil
	end
...
function Utils.IsAlive(char): boolean
	local hum = Utils.Humanoid(char)
	return hum ~= nil and hum.Health > 0
end

function Utils.IsAlly(char): boolean
	local flag = char and char:FindFirstChild("_is_ally")
	return flag ~= nil and (flag :: any).Value == true
end

function Utils.IsEnemy(player): boolean
	...
	if cfg and cfg.Aimbot and cfg.Aimbot.TeamCheck then
		if lp and lp.Team and player.Team and lp.Team == player.Team then
			return false
		end
	end
	...
	if Utils.IsAlly(char) then
		return false
	end
	return Utils.IsAlive(char)
end

function Utils.GetHitPart(char, mode: string?): BasePart?
	if not char then
		return nil
	end
	mode = mode or "Head"
	...
	-- Rivals uses HitboxHead/HitboxBody alongside Head
	return char:FindFirstChild("HitboxHead")
		or char:FindFirstChild(mode)
		or char:FindFirstChild("Head")
		or char:FindFirstChild("HumanoidRootPart")
end

function Utils.IsVisible(from: Vector3, target: BasePart, ignoreChar: Model?): boolean
...
function Utils.ScreenCenter(): Vector2
	local cam = Utils.Services.Workspace.CurrentCamera
...
|--!strict
|-- UI.luau | liv4cheets - Internal | ClickGUI
|-- Style refs: LinoriaLib tab/groupbox pattern (tabs + sections + controls),
|-- custom Electron-ish dark shell. No external deps. Themes: Default (volt) / Onyx.
...
local function cref<T>(i: T): T
	local f = (getgenv() :: any).cloneref
	if type(f) == "function" then
		local ok, o = pcall(f, i)
		if ok and o then
			return o
		end
	end
...
local function protect(gui: ScreenGui)
...
local function mk(class: string, props: any, parent: Instance?): any
	local o: any = Instance.new(class)
...
local function animsOn(): boolean
	local c = (getgenv() :: any).LIV4Config
	if c and c.Misc and c.Misc.Anims == false then
		return false
	end
	return true
end
...
function UI.New(): any
	...
	local rootStroke: any = mk("UIStroke", { Color = T.Stroke, Thickness = 1 }, root)
	do
		local sc = mk("UIScale", { Scale = 0.97 }, root)
		local cfg0 = (getgenv() :: any).LIV4Config
		if not cfg0 or cfg0.Misc.Anims ~= false then
			pcall(function()
				Tween:Create(sc, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 }):Play()
			end)
		else
			sc.Scale = 1
		end
	...
	do
		...
		bar.InputBegan:Connect(function(i)
			if i.UserInputType == Enum.UserInputType.MouseButton1 then
				drag, s, sp = true, i.Position, root.Position
			end
		end)
	...
	local win: any = { Gui = gui, Root = root, _pages = {}, _tabBtns = {} }

	function win:ApplyTheme(name: string)
		local th = THEMES[name] or THEMES.Default
		root.BackgroundColor3 = th.BG
		rootStroke.Color = th.Stroke
		for _, d in ipairs(root:GetDescendants()) do
			if d.Name == "PanelBG" and d:IsA("GuiObject") then
				(d :: any).BackgroundColor3 = th.Panel
			elseif d.Name == "CardBG" and d:IsA("GuiObject") then
				(d :: any).BackgroundColor3 = th.Card
			elseif d.Name == "AccentBG" and d:IsA("GuiObject") then
				(d :: any).BackgroundColor3 = th.Accent
			elseif d.Name == "MainText" and d:IsA("TextLabel") then
				(d :: any).TextColor3 = th.Text
			elseif d.Name == "DimText" and d:IsA("TextLabel") then
				(d :: any).TextColor3 = th.Dim
			elseif d.Name == "AccentLabel" then
				(d :: any).TextColor3 = th.AccentText
			end
		end
	end

	function win:SetMenuFont(name: string)
		local f = FONTS[name] or Enum.Font.GothamMedium
		for _, d in ipairs(root:GetDescendants()) do
			if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
				if (d :: any).Font ~= Enum.Font.Code then
					(d :: any).Font = f
				end
			end
		end
	...
	local function hover(btn: any, base: Color3)
		if not animsOn() then
			return
		end
		...
		btn.MouseEnter:Connect(function()
			pcall(function()
				Tween:Create(btn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundColor3 = hoverColor }):Play()
			end)
		end)
		btn.MouseLeave:Connect(function()
			pcall(function()
				Tween:Create(btn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundColor3 = base }):Play()
			end)
		end)
	end

	function win:Tab(name: string): any
		...
		function tab:Toggle(label: string, default: boolean, cb: (boolean) -> ())
			local st = default
		...
		function tab:Slider(label: string, min: number, max: number, default: number, cb: (number) -> ())
			local val = default
		...
		function tab:Dropdown(label: string, values: { string }, default: string, cb: (string) -> ())
			-- popup lives on the window root (never inside the scrolling canvas),
			-- positioned under the button, with an outside-click catcher.
			-- Pure dropdown list, no click-through cycling.
			local sel = default
			if not table.find(values, sel) then
				sel = values[1]
			end
			...
			b.MouseEnter:Connect(function()
				if animsOn() then
					pcall(function()
						Tween:Create(b, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundColor3 = T.Hover }):Play()
					end)
				end
			end)
			b.MouseLeave:Connect(function()
				if animsOn() then
					pcall(function()
						Tween:Create(b, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundColor3 = T.Card }):Play()
					end)
				end
			end)
			local catcher: any = nil
			local pop: any = nil
		...
		function tab:Textbox(label: string, placeholder: string, default: string, cb: (string) -> ())
		...
		function tab:Button(label: string, cb: () -> ())
...
--!strict
-- Aimbot.luau | liv4cheets Internal | Combat tab
-- v2: sticky target, velocity prediction, working wallcheck, dummy/NPC targets
-- (dummies let you test in the shooting range — players-only can't aim there).
...
function Aimbot.Start(Utils, Config)
	...
	local sticky: BasePart? = nil

	local function center(): Vector2
		local cam = S.Workspace.CurrentCamera
		local vs = cam and cam.ViewportSize or Vector2.new(1920, 1080)
		return Vector2.new(vs.X / 2, vs.Y / 2)
	end

	local function partVelocity(part: BasePart): Vector3
		local ok, v = pcall(function()
			return (part :: any).AssemblyLinearVelocity
		end)
		...
		if hrp then
			local ok2, v2 = pcall(function()
				return (hrp :: any).AssemblyLinearVelocity
			end)
	...
	local function visible(from: Vector3, part: BasePart): boolean
		local params = RaycastParams.new()
		params.FilterType = Enum.RaycastFilterType.Exclude
		...
		-- If the raycast hit something, check if it's part of the target model
		local hitModel = res.Instance:FindFirstAncestorOfClass("Model")
		if not hitModel then
			return false
		end
		-- Allow hit if it's the target's model or a descendant
		if hitModel == part.Parent then
			return true
		end
		-- Check if we hit the actual part or a descendant of the target
		if res.Instance == part or part:IsDescendantOf(hitModel) then
			return true
		end
		return false
	end

	local function validChar(ch: Instance): boolean
		local hum = (ch :: any):FindFirstChildOfClass("Humanoid")
		if not hum or hum.Health <= 0 then
			return false
		end
		if Utils.IsAlly(ch) then
			return false
		end
		return true
	end

	-- gather player chars + (optional) range dummies / NPC models
	local function targets(wantDummies: boolean): { Model }
		local out = {}
		...
		if wantDummies then
			...
			-- generic fallback: models grouped under a "Dummies"/"Targets" folder
			for _, n in ipairs({ "Dummies", "Targets", "Bots" }) do
				local f = S.Workspace:FindFirstChild(n)
				if f then
					for _, m in ipairs(f:GetChildren()) do
						if m:IsA("Model") and validChar(m) then
							table.insert(out, m)
						end
					end
				end
	...
	local function acquire(px: number, mode: string, wantDummies: boolean, wall: boolean): BasePart?
		local cam = S.Workspace.CurrentCamera
		if not cam then
			return nil
		end
		-- sticky: keep current part while held and still good (with margin)
		if sticky and sticky.Parent then
			...
			if m and validChar(m) then
				...
				if on and (Vector2.new(s.X, s.Y) - center()).Magnitude < px * 1.5 then
					if not wall or visible(cam.CFrame.Position, sticky) then
						return sticky
					end
				end
			end
		...
		local cc = center()
		-- Collect all valid targets with their screen positions
		local candidates: { [BasePart]: number } = {}
		for _, ch in ipairs(targets(wantDummies)) do
			...
			if d <= px then
				if wall and not visible(cam.CFrame.Position, part) then
					continue
				end
				candidates[part] = d
			end
	...
	-- ROOT FIX: default camera scripts overwrite Camera.CFrame every frame.
	-- A plain RenderStepped:Connect runs BEFORE that update, so our aim gets
	-- stomped (aimbot feels dead, esp. in first person). Camera.Value+1 runs
	-- after, so our CFrame is the one that renders. Works 1st + 3rd person.
	-- Improved: better target sorting, raycast visibility check, smoother lerp.
	RS:BindToRenderStep("LIV4Aimbot", Enum.RenderPriority.Camera.Value + 1, function()
		...
		fov.Position = cc
		fov.Radius = cb.AimFOV
		sFov.Visible = cb.SilentEnabled
		sFov.Position = cc
		sFov.Radius = cb.SilentFOV
		if not cb.AimbotEnabled then
			sticky = nil
			dot.Visible = false
			return
		end
		...
		-- velocity prediction so fast strafes don't lag behind
		...
		-- Smooth camera movement toward aim point
	...
	function Aimbot.SilentTarget(): Vector3?
		local cb = combat()
		if not cb.SilentEnabled then
			return nil
		end
		if math.random(100) > (cb.SilentChance or 100) then
			return nil
		end
	...
	function Aimbot.Destroy()
		pcall(function()
			RS:UnbindFromRenderStep("LIV4Aimbot")
		end)
		for _, d in ipairs({ fov, sFov, dot }) do
			pcall(function()
				d:Remove()
			end)
		end
...
--!strict
-- Triggerbot.luau | liv4cheets Internal | Combat tab
...
function Triggerbot.Start(Utils, Config, shootFn: (() -> ())?)
	...
	S.RunService.Heartbeat:Connect(function()
		...
		if not S.UserInputService:IsKeyDown(t.TriggerKey or Enum.KeyCode.T) then
			return
		end
		local cam = S.Workspace.CurrentCamera
		if not cam then
		...
		if not res or not HIT[res.Instance.Name] then
			return
		end
		...
		if not h or h.Health <= 0 or Utils.IsAlly(m) then
			return
		end
...
--!strict
-- ESP.luau | liv4cheets Internal | Visual tab
-- ROOT FIX for stuck drawings: central registry + destroy on teleport/leave/remove/unload.
-- Drawings are overlay-level: they survive teleports unless WE remove them.
...
function ESP.Start(Utils, Config)
	...
	local allDrawings: { any } = {}
	local conns: { RBXScriptConnection } = {}
	local function track(d: any)
		table.insert(allDrawings, d)
		return d
	end
	local function on(c: RBXScriptConnection)
		table.insert(conns, c)
	end

	type E = { box: any, fill: any, name: any, dist: any, hpBG: any, hp: any, skel: { any } }
	local cache: { [Player]: E } = {}
	local function get(p: Player): E
		local e = cache[p]
		if e then
			return e
		end
		local function sq(f: boolean): any
			return track(Drawing.new("Square") :: any)
		end
		local function tx(): any
	...
	local function dropPlayer(p: Player)
		local e = cache[p]
		if not e then
			return
		end
	...
	-- root fix: nuke overlay the moment we leave / teleport / get unloaded
	on(game:GetService("TeleportService").TeleportInit:Connect(function()
		hideAll()
		ESP.Destroy()
	end))
	...
	do
		local lp = Players.LocalPlayer
		if lp then
			on(lp.OnTeleport:Connect(function()
				hideAll()
				ESP.Destroy()
			end))
			if lp.Character then
				on(lp.CharacterRemoving:Connect(hideAll))
			end
			on(lp.CharacterAdded:Connect(function(ch)
				ch.AncestryChanged:Connect(function()
					if not ch:IsDescendantOf(game) then
						hideAll()
					end
				end)
			end))
	...
	local function chamsOn(ch: Model, color: Color3, on: boolean)
	...
	on(RS.RenderStepped:Connect(function()
		...
		for _, p in ipairs(Players:GetPlayers()) do
			...
			-- root fix: character gone from hierarchy (teleport/leave) -> drop, don't draw stale box
			if not ch or not ch:IsDescendantOf(game) then
				hide(e)
				continue
			end
			...
			if not hrp or not hum or hum.Health <= 0 then
				hide(e)
				chamsOn(ch, Color3.new(1, 1, 1), false)
				continue
			end
			if Utils.IsAlly(ch) and not v.ShowTeam then
				hide(e)
				chamsOn(ch, Color3.new(1, 1, 1), false)
				continue
			end
			...
			if d3 > (v.MaxDistance or 2000) then
				hide(e)
				continue
			end
			...
			e.name.Size = ts
			e.name.Text = p.Name
			e.name.Position = Vector2.new(b.X, y - ts - 2)
			e.name.Visible = v.Name
			e.dist.Size = ts
			e.dist.Text = "[" .. math.round(d3) .. "m]"
			e.dist.Position = Vector2.new(b.X, b.Y + 2)
			e.dist.Visible = v.Distance

			if v.HealthBar then
				local r = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
				e.hpBG.Size = Vector2.new(hbT, h)
				e.hpBG.Position = Vector2.new(x - hbT - 2, y)
				e.hpBG.Visible = true
				e.hp.Size = Vector2.new(hbT, h * r)
				e.hp.Position = Vector2.new(x - hbT - 2, y + h * (1 - r))
				e.hp.Color = Color3.fromRGB(255 - math.round(255 * r), math.round(255 * r), 60)
				e.hp.Visible = true
			else
				e.hpBG.Visible = false
				e.hp.Visible = false
			...
			if v.Skeleton then
				...
				for i, pr in ipairs(pairs) do
					...
					local L = e.skel[i]
					if a and bb then
						L.From, L.To, L.Visible = a, bb, true
					else
						L.Visible = false
					end
				end
...
--!strict
-- Ragebot.luau | liv4cheets Internal | Combat tab | HIGH BAN RISK, off by default
...
function Ragebot.Start(Utils, Config)
	...
	S.RunService.Heartbeat:Connect(function()
		...
		if not en or not vs or not remote then
			return
		end
		local my = S.Players.LocalPlayer and S.Players.LocalPlayer.Character
		if not my or not my:FindFirstChild("EquippedItem") then
			return
		end
		local cam = S.Workspace.CurrentCamera
		local best, bd = nil, math.huge
		for _, p in ipairs(S.Players:GetPlayers()) do
			if p == S.Players.LocalPlayer then
				continue
			end
			local ch = p.Character
			if not ch or not Utils.IsAlive(ch) or Utils.IsAlly(ch) then
				continue
			end
			local hrp = ch:FindFirstChild("HumanoidRootPart") :: BasePart?
			if hrp and cam then
